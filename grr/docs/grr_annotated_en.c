...
typedef struct Edge {
    ...
    int next; // index of next edge in the linked list
} Edge;

// Adds a directed edge u -> v to the given adjacency list
void addEdge(int u, int v, Edge *edges, int *head, int *edgeCount) {
    edges[++*edgeCount] = (Edge){v, head[u]}; // create new edge
    head[u] = *edgeCount; // point head[u] to new edge
}
...
int *ids, id = 1; // discovery time of each node
int *low; // low-link value
char *onStack; // boolean flag array
int *stack, stackTop = 0;

int *scc_ids, scc_cnt = 0; // component id of each node, and SCC count

// Tarjan DFS to identify strongly connected components
void scc_dfs(int u) {
    stack[stackTop++] = u;
    onStack[u] = 1;
    ids[u] = low[u] = id++;
    ...
    if (ids[u] == low[u]) {
        ...
        do {
            cur = stack[--stackTop];
            onStack[cur] = 0;
            scc_ids[cur] = scc_cnt; // assign component number
        } while (cur != u);
    }
}


// Pair of SCCs representing edges in the condensation DAG
typedef struct {
    int u, v;
} SCCEdge;
...
// Compare function for sorting SCCEdge array
...
// Arrays for topological order
int *topo, topo_cnt;
char *topo_visited;

// DFS to produce topological ordering of DAG
void topo_dfs(int u, Edge *edges, int *head) {
    ...
    topo[topo_cnt++] = u; // append after visiting descendants
}


// Simple linked list node for Set implementation
typedef struct Node {
    int val;
    struct Node *next;
} Node;
...
typedef struct {
    Node *list; // linked list of current elements
    int *hashtable; // array storing keys or 0 for empty
...
// Initialize Set with small capacity
void set_init(Set *s) {
...
int main() {
    ...
    // allocate raw graph arrays and read edges
    ...
    // allocate Tarjan arrays and run SCC
    ...
    // collect edges between different SCCs
    ...
    // build forward & backward DAG adjacency lists
    ...
    // topological sort of DAG (reverse postorder)
    ...
    // forward pass counts descendants in DAG
    ...
    // backward pass counts ancestors in DAG
    ...
    // collect final answers: those with full coverage
